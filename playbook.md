...
### 49. 平行列表按下标同步取值 = 定时炸弹（r48）
- **坑**：归一化"抢救步骤数组"只填 steps 没填 estimates，而预览/应用按 `estimates.get(i)`
  与 steps 同步下标取值 → 触发抢救路径（自定义提示词的非内置 JSON）即越界闪退，
  表现为"偶尔闪退"（只有特定输出结构才踩中）。
- **规则**：**任何按索引配对消费的平行列表，要么写入端保证等长，要么消费端越界取空**——
  两道都做；新代码路径往老消费逻辑里喂数据时，先审计老逻辑的全部隐含假设。
